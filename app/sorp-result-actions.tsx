...
export function SorpResultActions({ sessionId, organisation, income, result, children, onBackToAssessment }: { sessionId: string; organisation: string; income: string; result: ResultSummary; children: ReactNode; onBackToAssessment: () => void }) {
  ...
  return <main className={`sorp-completion-flow${reportOpen ? " is-report-open" : ""}`}>
    {!reportOpen && <>
      <section className="sorp-completion-event" aria-labelledby="completion-title"><div className="sorp-completion-sweep" aria-hidden="true" /><button type="button" className="sorp-back-to-assessment" onClick={onBackToAssessment}>← Back to assessment</button><p>✓ 7 stages complete · Stage 8 of 8</p><h1 id="completion-title">Your Full Readiness Review</h1><strong>Your developed SORP readiness assessment is ready.</strong><ol aria-label="Seven assessment stages complete; Full Readiness Review active">{Array.from({ length: 8 }, (_, index) => <li key={index} className={index === 7 ? "is-review-stage" : ""}><span>{index === 7 ? "8" : "✓"}</span></li>)}</ol></section>
