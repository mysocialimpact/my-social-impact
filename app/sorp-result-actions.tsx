...
type ResultSummary = { score: number; band: string; overview: string; strong: string[]; attention: string[]; priorities: string[] };
...
export function SorpResultActions({ sessionId, organisation, income, result, children }: { sessionId: string; organisation: string; income: string; result: ResultSummary; children: ReactNode }) {
  ...
  return <main className={`sorp-completion-flow${reportOpen ? " is-report-open" : ""}`}>
    {!reportOpen && <>
      <section className="sorp-completion-event" aria-labelledby="completion-title"><div className="sorp-completion-sweep" aria-hidden="true" /><p>✓ 7 of 7 complete</p><h1 id="completion-title">You’ve finished your SORP readiness check</h1><strong>Your personalised report is ready.</strong><ol aria-label="All seven stages complete">{Array.from({ length: 7 }, (_, index) => <li key={index}><span>✓</span></li>)}</ol></section>
      <section className="sorp-result-preview"><header><div><span>Your SORP readiness result</span><h2>{result.band}</h2><p>{result.overview}</p></div><div><strong>{result.score}</strong><span>/ 100</span></div></header><div className="sorp-result-preview-grid"><article><h3>What already looks strong</h3><ul>{strongest.map((item) => <li key={item}>{item}</li>)}</ul></article><article><h3>Important gaps</h3><ul>{gaps.map((item) => <li key={item}>{item}</li>)}</ul></article></div><p className="sorp-report-ready">Your full personalised report is ready.</p></section>
