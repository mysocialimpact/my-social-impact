...
export function SorpReadinessConversation({ setupOnly = false, onSetupComplete }: { setupOnly?: boolean; onSetupComplete?: (state: ReadinessState, workflow: ReadinessWorkflow) => void }) {
  ...
  if (!setupOnly && result && state.score !== null && sessionId && reviewIndex === null) {
    ...
    if (stageEightReportMode) return <div className="readiness-chat is-result-mode">
      <SorpResultActions sessionId={sessionId} organisation={state.charityName} income={state.setup.income} result={result} openOnEntry onBackToAssessment={() => setStageEightReportMode(false)}>
  ...
  return <div className="readiness-chat">
    ...
    <form className={`readiness-composer${activityQuestion ? " is-activity-composer" : ""}${activitySelectionCount ? " has-activity-selections" : ""}${reviewIndex !== null ? " is-reviewing" : ""}${isStageOnePayoff ? " is-stage-one-payoff" : ""}`} onSubmit={submit}>
      ...
      <label htmlFor="readiness-answer" aria-live="polite">{isStageOnePayoff ? "ANYTHING YOU’D LIKE TO ADD, CLARIFY OR ASK BEFORE YOU SEE YOUR QUICK REVIEW?" : isReadinessScale ? "WANT TO ADD ANYTHING, OR FANCY A QUICK CHAT BEFORE YOU SUBMIT THIS ANSWER?" : `Or tell us in your own words — or ask ${impactMode ? "an impact" : "a SORP"} question.`}</label>
      ...
      <div className="readiness-submit-row"><button type="button" className="readiness-mic" onClick={recordingState === "recording" ? stopRecording : () => void startRecording()} disabled={busy || quickAdvancing || recordingState === "transcribing"}>{recordingState === "recording" ? `Stop · ${recordingTime(recordingSeconds)}` : recordingState === "transcribing" ? "Transcribing…" : "Use microphone"}</button>{(!isStageOnePayoff || composer.trim().length >= 2) && <button type="submit" className={busy || quickAdvancing ? "is-working" : undefined} disabled={busy || quickAdvancing || (!isReadinessScale && !pendingStructuredAnswer && !(activityQuestion && activitySelections.length) && composer.trim().length < 2) || isReadinessScale && !activeQuickAction && !suggestedQuickAction && !composer.trim() || recordingState !== "idle"}>{busy ? "Understanding…" : quickAdvancing ? "Saving…" : isStageOnePayoff ? "Send message" : isReadinessScale ? "Continue" : reviewIndex !== null ? "Save revised answer" : conversationFirstMessage ? "Send message" : activityQuestion && activitySelections.length ? "Continue with choices" : result ? "Keep talking" : "Continue"} <span>→</span></button>}</div>
      </section>
    </form>
    {saveDialog}
  </div>;
