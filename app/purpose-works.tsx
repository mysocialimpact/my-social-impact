...
const pillars = [
  ...
  ["03", "Data", "The evidence behind the organisation’s claims.", "evidence", "/assets/excellence/data-icon.png"],
    ...
    <>
      ...
      <main className="pw-page">
        ...
        <section className="pw-excellence pw-pad" id="excellence">
          ...
          <div className="pw-excellence-head" data-reveal><h2>Purpose Works is built on Social Impact Excellence.</h2><div><p><a href="/social-impact-excellence">Social Impact Excellence</a> is My Social Impact’s flagship methodology. It brings Purpose, Leadership, Data, Delivery and Communications together as one practical approach.</p><p><strong>Purpose Works delivers directly across Purpose, Leadership and Communications.</strong> Data and Delivery ground the work in evidence and practical delivery.</p><a className="pw-excellence-link" href="/social-impact-excellence">Explore Social Impact Excellence <span>↗</span></a></div></div>
          <div className="pw-pillars" data-reveal>{pillars.map(([number, name, copy, role, icon]) => <article className={"is-" + role} key={name}><span>{number}</span><div className="pw-pillar-icon"><Image src={icon} alt="" width={260} height={260} unoptimized /></div><h3>{name}</h3><p>{copy}</p><small>{role === "evidence" ? "Foundation for the work" : "Purpose Works focus"}</small></article>)}</div>
